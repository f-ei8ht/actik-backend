...
export async function resolveDefaultBranch(repo: RepoRef): Promise<string> {
  try {
    const response = await fetch(
      `https://api.bitbucket.org/2.0/repositories/${repo.owner}/${repo.name}`
    )
    if (!response.ok) return FALLBACK_BRANCH
    const doc = (await response.json()) as BitbucketRepoDoc
    return doc.mainbranch?.name ?? FALLBACK_BRANCH
  } catch {
...
export function rawUrl(repo: RepoRef, branch: string, path: string): string {
  return `https://bitbucket.org/${encodeURIComponent(repo.owner)}/${encodeURIComponent(repo.name)}/raw/${encodeURIComponent(branch)}/${path}`
}
...
export async function fetchRawFile(
  ...
  if (!response.ok) {
    throw new AppError(502, 'repo_fetch_failed', `bitbucket returned ${response.status} for ${path}`)
  }
